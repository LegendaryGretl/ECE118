...
static TowerEncirclementFSMState_t CurrentState = InitPSubState; // <- change name to match ENUM
...
uint8_t InitReverseTowerEncirclementFSM(void) {
    ES_Event returnEvent;

    CurrentState = InitPSubState;
    returnEvent = RunReverseTowerEncirclementFSM(INIT_EVENT);
...
ES_Event RunReverseTowerEncirclementFSM(ES_Event ThisEvent) {
    static int marker = 0;
    ...
    if (makeTransition == TRUE) { // making a state transition, send EXIT and ENTRY
        ...
        RunReverseTowerEncirclementFSM(EXIT_EVENT); // <- rename to your own Run function
        CurrentState = nextState;
        RunReverseTowerEncirclementFSM(ENTRY_EVENT); // <- rename to your own Run function
    }
