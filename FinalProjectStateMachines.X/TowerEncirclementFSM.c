...
uint8_t InitTowerEncirclementFSM(void) {
    ES_Event returnEvent;

    CurrentState = InitPSubState;
    returnEvent = RunTowerEncirclementFSM(INIT_EVENT);
...
//            break;
    ...
    if (makeTransition == TRUE) { // making a state transition, send EXIT and ENTRY
        ...
        RunTowerEncirclementFSM(EXIT_EVENT); // <- rename to your own Run function
        CurrentState = nextState;
        RunTowerEncirclementFSM(ENTRY_EVENT); // <- rename to your own Run function
    }
