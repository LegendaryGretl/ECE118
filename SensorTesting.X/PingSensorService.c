...
uint8_t InitPingSensorService(uint8_t Priority) {
    ...
    PING_SENSOR_ENABLE_TRIS = 0;
    PING_SENSOR_ECHO_TRIS = 1;
...
ES_Event RunPingSensorService(ES_Event ThisEvent) {
    ...
    switch (ThisEvent.EventType) {
        case ES_INIT:
            ...
            //
            PING_SENSOR_ENABLE = 0;
            // This section is used to reset service for some reason
            break;

        case ES_GET_PING_SENSOR_DISTANCE:
            // set ping sensor enable pin high
            PING_SENSOR_ENABLE = 1;
            ES_Timer_InitTimer(PING_SENSOR_ENABLE_TIMER, 1);
            break;
        ...
        case ES_TIMEOUT:
            if (ThisEvent.EventParam != PING_SENSOR_ENABLE_TIMER){
                break;
            }
            PING_SENSOR_ENABLE = 0;
            break;

        case ES_PING_SENSOR_PULSE_LOW:
